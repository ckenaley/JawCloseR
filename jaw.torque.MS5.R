...
j[n,"A3v"] =j[n,"A3dML"]/A3MLOpen/TimeStepSecs;
...
j[n,"A3Work"]=j[n,"A3Force"]*	j[n,"A3dML"]/Scale
j[n,"A3Power"]=j[n,"A3Work"]/TimeStepSecs
j[n,"A3relL"]=j[n,"A3ML"]/A2MLOpt



#************* A2 force ********
j[n,"A2dML"] =j[n-1,"A2ML"]-j[n,"A2ML"];
j[n,"A2v"] =j[n,"A2dML"]/A2MLOpen/TimeStepSecs;
...
j[n,"A2Work"]=j[n,"A2Force"]*	j[n,"A2dML"]/Scale
j[n,"A2Power"]=j[n,"A2Work"]/TimeStepSecs
j[n,"A2relL"]=j[n,"A2ML"]/A2MLOpt


#************* Aw force ********
j[n,"AwdML"] =j[n-1,"AwML"]-j[n,"AwML"];
j[n,"Awv"] =j[n,"AwdML"]/AwMLOpen/TimeStepSecs;
...
ifelse(j[n,"AwFL"] >AwFLOpt,j[n,"AwFfl"]<- -6.25*(j[n,"AwFL"]/AwFLOpt)^2+12.5*(j[n,"AwFL"]/AwFLOpt)-5.25,j[n,"AwFfl"]  <- 1) # length-tension relationship from Van Wassenbergh
...
j[n,"AwWork"]=j[n,"AwForce"]*	j[n,"AwdML"]/Scale
j[n,"AwPower"]=j[n,"AwWork"]/TimeStepSecs
j[n,"AwrelL"]=j[n,"AwML"]/A2MLOpt
...
j[n,"A3AwTheta"] <-  cos.ang(j[n,"AwML"],j[n,"A3ML"],dist.2d(A3Or,AwOr))
...
if(progress==T){
  if(i %in% c(2,seq(print.every,MaxIts,print.every))){
    ...
    suppressWarnings( p <- ggplot()+geom_point(data=geom.df,aes(x=x,y=y, colx=var),size=3)+theme_classic(20)+ylim(-4,4)+line.2pt("joint.p","JawTip",geom.df)+line.2pt("resOr","nexus",geom.df,col="gray")+line.2pt("inlever","nexus",geom.df,col="blue")+line.2pt("nexus","AwIns",geom.df,c="darkgreen")+line.2pt("A3Or","nexus",geom.df,c="maroon")+line.2pt("A2Or","nexus",geom.df,col="red")+xlim(-4,4)+geom_point(aes(x=x,y=y),data=circs,size=0.5)+theme(legend.position="none")+geom_text(data=n.dat,aes(x=x-1.25,y=y,label=paste0(name,": ",dat)),size=2,hjust=0)
    )
    print(p)
